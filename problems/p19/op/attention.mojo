...
struct AttentionCustomOp:
    ...
    fn execute[
        ...
        if target == "gpu":
            ...
            alias transpose_threads_per_block = (TRANSPOSE_BLOCK_DIM_X, TRANSPOSE_BLOCK_DIM_Y)
            # Tile over the K (d, seq_len) matrix
            alias transpose_blocks_per_grid = (
                (d + TRANSPOSE_BLOCK_DIM_X - 1) // TRANSPOSE_BLOCK_DIM_X,
                (seq_len + TRANSPOSE_BLOCK_DIM_Y - 1) // TRANSPOSE_BLOCK_DIM_Y
            )
